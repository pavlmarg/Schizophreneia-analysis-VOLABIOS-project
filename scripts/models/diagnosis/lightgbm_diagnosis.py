...
lgbm_core = lgb.LGBMClassifier(class_weight='balanced', random_state=10, n_jobs=-1, verbose=-1, learning_rate=0.02, n_estimators=150, max_depth=3)
...
plt.title('Recursive Feature Elimination (RFECV)', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('outputs/rfecv_plot.png', dpi=150)
plt.close()
...
fig.suptitle('VOLABIOS: F1-Optimized Diagnostic Model', fontsize=14, fontweight='bold')
...
plt.figure(figsize=(10, 6))
plt.title("SHAP Summary Plot: Global Feature Drivers", fontsize=14, fontweight='bold')
