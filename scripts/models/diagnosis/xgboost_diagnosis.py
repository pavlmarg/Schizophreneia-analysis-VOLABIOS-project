...
# 5 seeds × 5-fold = 25 total evaluation folds
SEEDS = [0, 7, 21, 42, 84]
...
for seed in SEEDS:
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(outer_skf.split(X_full, y_full)):
        ...
        imputer_cont = IterativeImputer(estimator=RandomForestRegressor(n_estimators=50, random_state=seed), random_state=seed)
        X_train_cont_imp = pd.DataFrame(imputer_cont.fit_transform(X_train_cont), columns=CONTINUOUS_FEATURES)
        X_val_cont_imp = pd.DataFrame(imputer_cont.transform(X_val_cont), columns=CONTINUOUS_FEATURES)

        imputer_cat = IterativeImputer(estimator=RandomForestClassifier(n_estimators=50, random_state=seed), random_state=seed)
        X_train_cat_imp = pd.DataFrame(imputer_cat.fit_transform(X_train_cat), columns=new_cat_features)
        X_val_cat_imp = pd.DataFrame(imputer_cat.transform(X_val_cat), columns=new_cat_features)
        ...
        if len(filtered_features) == 0:
            filtered_features = CONTINUOUS_FEATURES

        X_train_filtered = X_train[filtered_features]
        X_val_filtered = X_val[filtered_features]

        # --- FEATURE SELECTION B: forward sequential selection (XGBoost, f1_macro) ---
        ...
        def objective(trial):
            ...
            for inner_train_idx, inner_val_idx in inner_cv.split(X_train_opt_scaled, y_train_opt_smote):
                clf_t = xgb.XGBClassifier(**params)
        ...
        for i_tr_idx, i_val_idx in inner_skf.split(X_train_final, y_train):
            ...
            clf_i = xgb.XGBClassifier(**best_params)
...
auc_distances = [abs(s['auc'] - mean_auc) for s in fold_snapshots]
median_fold_idx = int(np.argmin(auc_distances))
...
robust_df.to_csv('outputs/feature_selection_frequency_xgboost.csv', index=False)

# RETRAIN ON FULL DATA using median fold's best params FOR SHAP
...
X_full_cont = X_full[CONTINUOUS_FEATURES].copy().reset_index(drop=True)
...
X_full_smote, y_full_smote = smote_full.fit_resample(X_full_final, y_full)

scaler_full = MinMaxScaler()
X_full_scaled = pd.DataFrame(
    scaler_full.fit_transform(X_full_smote),
    columns=median_final_features)

# Fit final model
clf_full = xgb.XGBClassifier(**median_best_params)
clf_full.fit(X_full_scaled, y_full_smote)

# Original scaled data used for SHAP background
X_orig_scaled = pd.DataFrame(scaler_full.transform(X_full_final), columns=median_final_features)
...
fig.suptitle(f'XGBoost Model — Median Fold ({fold_label})', fontsize=14, fontweight='bold')
...
axes[1].plot(fpr, tpr, color='blue', lw=2, label=f'AUC = {median_snap["auc"]:.3f} (median fold)')
...
rel_imp = importances / importances.sum() if importances.sum() > 0 else importances
...
axes[2].barh(imp_df['feature'], imp_df['importance'], color='blue')
...
plt.tight_layout()
plt.savefig('outputs/diagnosis_xgboost.png', dpi=150, bbox_inches='tight')
plt.close()

# SHAP — full-data model on original scaled data
explainer_full = shap.TreeExplainer(clf_full, data=X_orig_scaled)
shap_values_full = explainer_full.shap_values(X_orig_scaled)

shap_values_pos = (shap_values_full[1]if isinstance(shap_values_full, list)else shap_values_full)

plt.figure(figsize=(10, 6))
plt.title("SHAP Summary Plot: Selected Features (XGBoost — full-data model)", fontsize=14, fontweight='bold')
shap.summary_plot(shap_values_pos, X_orig_scaled, show=False)
plt.tight_layout()
plt.savefig('outputs/shap_summary_xgboost.png', dpi=150, bbox_inches='tight')
plt.close()
...
pd.DataFrame([summary]).to_csv('outputs/summary_xgboost.csv', index=False)
